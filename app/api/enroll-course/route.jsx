...
export async function GET(req) {
  ...
  if (courseId) {
    const result = await db.select().from(coursesTable).innerJoin(enrollCourseTable,
...
export async function PUT(req) {
  ...
  const result = await db.update(enrollCourseTable).set({
    ...
    eq(enrollCourseTable.userEmail, user?.primaryEmailAddress.emailAddress))).returning(enrollCourseTable)
